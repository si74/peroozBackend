...
/**
 * Script to grab contributor id from session token
 * Author: Sneha Inguva
 * Date: 10-27-2014
 */
...
function get_contributor(){
	...
	$stmt = "Select PeroozContributorId From Contributors Where UserId=(Select UserId From UserSessions Where UserSessionToken=:UserSessionToken)";
	...
	if ($query_result){
		...
		echo json_encode(array('message' => 'OK', 'perooz_contributor_id' => $query_result[0]['PeroozContributorId']));
	}else{
		http_response_code(200);
