...
function check_login($client_id,$request_method,$nonce,$db_settings){
	...
	$sess_token = $uid['uuid()'];
	$sess_hash = md5($sess_token); //hash session_token value using crypt to store in db
	$expiry_time = date('Y-m-d H:i:s', strtotime('+14 days'));
	...
	$result = $con->insert($stmt_sess,$stmt_sess_values,$stmt_sess_prop,false,'UserSession');
